...
def dohoteltask(taskname, parameter):
    ...
    with dbCon:
        ...
        if dbExists:
            if taskname == "clean":
                roomsToClean = cursor.execute("""Select Rooms.RoomNumber ASC
                                                 from Rooms
                                                 EXCEPT SELECT Residents.RoomNumber FROM Residents""").fetchall()
                        ...
                        if room == roomsToClean[len(roomsToClean)-1]:
                            ...
                            cursor.execute("""update TaskTimes
                                              ...
                                              Where TaskTimes.TaskId = (
                                                      select TaskId
                                                      from Tasks
                                                      where TaskName='clean')""")
                            return timeToReturn
                        else:
                            print(str(room[0]) + ", "),
            else:
                person = cursor.execute("""Select FirstName,LastName
                                           from Residents
                                           Where RoomNumber = ?""",(parameter,)).fetchone()
                    ...
                    if taskname == "breakfast":
                        ...
                        cursor.execute("""update TaskTimes set NumTimes = NumTimes-1
                                          Where TaskTimes.TaskId = (
                                                   select TaskId
                                                   from Tasks
                                                   where TaskName='breakfast' and Parameter=?)""",(parameter,))
                        return timeToReturn
                    else:
                        timeToReturn = round(time.time(),2)
                        ...
                        cursor.execute("""update TaskTimes
                                          ...
                                          Where TaskTimes.TaskId = (
                                                  select TaskId
                                                  from Tasks
                                                  where TaskName='wakeup' and Parameter=?)""",(parameter,))
                        return timeToReturn